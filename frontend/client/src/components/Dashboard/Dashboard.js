...
import { getArrForSelect } from '../../utils/dashboardUtils';
...
class Dashboard extends Component {
  ...
  render() {
    let propData = {};
    let useProps = false;
    if ('dataEval' in this.props.dashboard) {
      console.log('hello I am redux', this.props.dashboard);
      propData = this.props.dashboard;
      useProps = true;
    }
    ...
    if (useProps) {
      return (
        <div>
          ...
          <SideNavbar />
          <DashboardNav
            totals={propData.navData}
            userList={getArrForSelect(_.union(propData.data.owesList, propData.data.owedList))}
          />
          <Duelist data={propData.data} fresh={propData.dataEval} />
        </div>
      );
    }
    return (
